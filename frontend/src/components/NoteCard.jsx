...
const NoteCard = ({ note, onEdit, onDelete, onUpdate }) => {
  ...
  const toggleChecklistItem = async (idx) => {
    ...
    try {
      const res = await updateNote(note._id, { checklist: updatedChecklist });
      
      if (onUpdate && res.data.status === 'Completed' && note.status !== 'Completed') {
        // Wait briefly for background AI generation to finish before fetching
        setTimeout(() => {
          onUpdate();
        }, 2500);
      }
    } catch (error) {
      console.error("[NOTE CARD] Failed to update checklist:", error);
      // Revert on error
            ...
            <div className="space-y-1">
              <div className="pt-2">
                ...
                {note.datewiseProgress && note.datewiseProgress.length > 0 && (
                  <div className="h-[100px] mt-4 -mx-1 bg-gray-50 dark:bg-gray-800/50 rounded-lg p-2 border border-gray-100 dark:border-gray-700 w-full relative">
